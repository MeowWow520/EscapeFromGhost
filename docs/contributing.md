...
感谢您对 Escape From Ghost 项目的关注！我们欢迎所有形式的贡献，包括但不限于代码、文档、翻译、测试、Bug 报告和功能建议。
...
在参与本项目时，请遵守我们的[行为准则](../CODE_OF_CONDUCT.md)。我们致力于为所有人提供一个开放、友好、包容的社区环境。
...
- **编译器**: 支持 C++17 或更高版本的 MSVC 编译器
- **CMake**: 版本 3.10 或更高
- **构建工具**:
  - Windows: Visual Studio Code或 Ninja
  - Linux: Make 或 Ninja
  - macOS: Xcode 或 Ninja
...
```bash
# 克隆仓库
git clone https://github.com/yourusername/EscapeFromGhost.git
cd EscapeFromGhost
...
# 运行游戏
Release\EscapeFromGhost-Windows.exe
```
 ...
 * @return 返回值描述
 */
int calculateScore(int paramName);
```
...
所有贡献者都将被添加到 [CONTRIBUTORS](../CONTRIBUTING.md) 列表中。我们感谢每一位为项目做出贡献的人！

---

再次感谢您的贡献！让我们一起把 Escape From Ghost 变得更好！🎮✨